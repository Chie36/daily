...
class Lqr:
    def __init__(self):
        ...
        self.B = np.eye(self.n_state)

        self.Q = np.eye(self.n_state)
        self.R = np.eye(self.n_control)

        self.P = np.zeros((self.N + 1, self.n_state, self.n_state))
        self.K = np.zeros((self.N, self.n_control, self.n_state))
    ...
    def solve_lqr(self):
        ...
        for k in range(self.N - 1, -1, -1):
            ...
            S = self.R + self.B.T @ self.P[k + 1] @ self.B
            self.P[k] = (
                self.Q
                + self.A.T @ self.P[k + 1] @ self.A
                - self.A.T
                @ self.P[k + 1]
                @ self.B
                @ np.linalg.inv(S)
            ...
            self.K[k] = np.linalg.inv(S) @ self.B.T @ self.P[k + 1] @ self.A
        ...
        for k in range(self.N):
            ...
            self.u_traj[k] = -self.K[k] @ self.x_traj[k]
            # State update
